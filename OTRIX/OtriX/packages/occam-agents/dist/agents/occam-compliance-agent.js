...
exports.OCCAMComplianceAgent = void 0;
...
class OCCAMComplianceAgent {
    constructor(factBoxService, auditService, secureVault) {
        this.factBoxService = factBoxService || new FactBoxService_1.FactBoxService();
        this.auditService = auditService || new AuditService_1.AuditService();
        this.secureVault = secureVault || new SecureVault_1.SecureVault();
    ...
    async analyzeRegulation(document) {
        ...
        try {
            ...
            await this.auditService.logFailure(traceId, 'analyze_regulation', error instanceof Error ? error.message : 'Unknown error', { document_length: document.length });
            throw error;
        }
    }
    /**
     ...
     */
    async verifyRequirements(entityId) {
        const traceId = this.auditService.generateTraceId();
        const errors = [];
        const warnings = [];
        try {
            ...
            if (!entity) {
                ...
                await this.auditService.logFailure(traceId, 'verify_requirements', 'Entity not found', { entity_id: entityId });
                return {
                    valid: false,
                    errors,
                    warnings,
            ...
            await this.auditService.logFailure(traceId, 'verify_requirements', error instanceof Error ? error.message : 'Unknown error', { entity_id: entityId });
            throw error;
        }
    }
    /**
        ...
        try {
            ...
            if (!action.requirementsVerified) {
                ...
                if (!validation.valid) {
                    ...
                    await this.auditService.logFailure(traceId, 'execute_filing', 'Validation failed', { action_id: action.id, errors: result.errors });
                    return result;
                }
            }
            // In production, this would submit to actual regulatory APIs
            // using credentials from SecureVault
            const filingId = (0, uuid_1.v4)();
            ...
            await this.auditService.logFailure(traceId, 'execute_filing', error instanceof Error ? error.message : 'Unknown error', { action_id: action.id });
            return {
                success: false,
                timestamp: new Date(),
                trace_id: traceId,
     ...
     */
    async generateComplianceReport(entityId, regulation) {
        const traceId = this.auditService.generateTraceId();
        const findings = [];
        try {
            // Verify requirements
            ...
            await this.auditService.logFailure(traceId, 'generate_compliance_report', error instanceof Error ? error.message : 'Unknown error', { entity_id: entityId, regulation });
            throw error;
        }
    }
    /**
     ...
     */
    parseRegulatoryDocument(document) {
     ...
     */
    generateNextSteps(validation, findings) {
        const steps = [];
...
exports.OCCAMComplianceAgent = OCCAMComplianceAgent;
//# sourceMappingURL=occam-compliance-agent.js.map