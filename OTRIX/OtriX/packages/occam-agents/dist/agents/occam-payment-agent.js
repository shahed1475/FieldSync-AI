...
exports.OCCAMPaymentAgent = void 0;
const services_1 = require("../services");
class OCCAMPaymentAgent {
    constructor() {
        this.initialized = false;
        // Initialize audit service first (needed by others)
        this.auditService = new services_1.AuditService(process.env.AUDIT_LOG_PATH || './logs', process.env.LOG_LEVEL || 'info');
        ...
        this.vaultService = new services_1.VaultService(vaultConfig);
        // Initialize governance service
        this.governanceService = new services_1.GovernanceService(this.auditService);
        // Initialize payment service
        this.paymentService = new services_1.PaymentService(this.vaultService, this.auditService);
        // Initialize FactBox service
        this.factBoxService = new services_1.FactBoxService(this.auditService);
    }
    /**
    ...
    async initialize() {
        ...
        try {
            ...
            await this.auditService.logEvent('payment.initiated', 'OCCAM Payment Agent initialized successfully', {
                services: [
                    'VaultService',
                    'AuditService',
                    'GovernanceService',
            ...
            await this.auditService.logEvent('payment.failed', 'OCCAM Payment Agent initialization failed', { error: error.message }, { severity: 'critical', success: false, errorMessage: error.message });
            throw error;
        }
    }
    /**
    ...
    async initiatePayment(paymentDetails) {
        ...
        try {
            ...
            await this.auditService.addToTrail(transactionId, 'payment.initiated', 'Validating transaction against governance policies', { context });
            ...
            if (!governanceResult.allowed) {
                ...
                await this.auditService.addToTrail(transactionId, 'payment.failed', 'Payment blocked by governance policies', { violations: governanceResult.violations }, { severity: 'warning', success: false, errorMessage: error });
            ...
            if (governanceResult.requiresApproval) {
                await this.auditService.addToTrail(transactionId, 'approval.requested', 'Payment requires approval', { approvalRequestId: governanceResult.approvalRequestId }, { severity: 'warning' });
            ...
            // Process payment
            await this.auditService.addToTrail(transactionId, 'payment.processing', 'Processing payment through payment service', { paymentDetails });
            const paymentResult = await this.paymentService.initiatePayment({
                ...paymentDetails,
                idempotencyKey: transactionId,
            });
            // Update audit trail with result
            await this.auditService.addToTrail(transactionId, paymentResult.success ? 'payment.completed' : 'payment.failed', paymentResult.success ? 'Payment processed successfully' : 'Payment processing failed', { paymentResult }, {
                severity: paymentResult.success ? 'info' : 'error',
                success: paymentResult.success,
                errorMessage: paymentResult.error,
            });
            // Complete audit trail
            await this.auditService.completeTrail(transactionId, paymentResult.success ? 'completed' : 'failed');
            return {
                ...paymentResult,
                transactionId,
            };
            ...
            await this.auditService.addToTrail(transactionId, 'payment.failed', 'Payment failed with exception', { error: error.message }, { severity: 'error', success: false, errorMessage: error.message });
    ...
    async verifyPaymentStatus(transactionId) {
        ...
        try {
            ...
            await this.auditService.logEvent('payment.verified', 'Payment verification completed', { transactionId, verification }, { traceId: transactionId, success: verification.isVerified });
            return verification;
        }
        catch (error) {
            await this.auditService.logEvent('payment.verified', 'Payment verification failed', { transactionId, error: error.message }, {
    ...
    async logReceipt(receipt) {
        this.ensureInitialized();
        try {
            await this.auditService.logEvent('payment.completed', 'Payment receipt logged', { receipt }, {
            ...
            await this.auditService.logEvent('payment.failed', 'Failed to log payment receipt', { receipt, error: error.message }, {
                traceId: receipt.transactionId,
                severity: 'error',
                success: false,
                errorMessage: error.message,
     ...
     */
    async renewSubscription(entityId, renewalData) {
        this.ensureInitialized();
        try {
            ...
            const amount = renewalData?.amount ||
                ...
                await this.factBoxService.getPaymentAmount(entityId, renewalLicense?.id, renewalLicense?.issuingAuthority);
            if (!amount) {
            ...
            await this.auditService.logEvent('renewal.scheduled', 'Renewal payment initiated', { entityId, paymentData }, { severity: 'info', resource: entityId });
            // Process payment
            ...
            if (result.success && renewalLicense) {
                ...
                await this.auditService.logEvent('renewal.processed', 'License renewed successfully', { entityId, licenseId: renewalLicense.id, newExpiryDate }, { severity: 'info', resource: entityId });
            }
            return result;
        }
        catch (error) {
            await this.auditService.logEvent('renewal.processed', 'Renewal payment failed', { entityId, error: error.message }, {
                severity: 'error',
                resource: entityId,
                success: false,
                errorMessage: error.message,
...
exports.OCCAMPaymentAgent = OCCAMPaymentAgent;
//# sourceMappingURL=occam-payment-agent.js.map