...
exports.OCCAMConsultancyAgent = void 0;
 ...
 */
class OCCAMConsultancyAgent {
    constructor(factBox, auditService, secureVault, logger, llmConfig, storageDir) {
        this.factBox = factBox || new FactBoxService_1.FactBoxService();
        this.auditService = auditService || new AuditService_1.AuditService();
        // SecureVault requires encryption key
        if (!secureVault) {
            const defaultKey = process.env.ENCRYPTION_KEY || '0'.repeat(64);
            this.secureVault = new SecureVault_1.SecureVault(defaultKey);
        }
        else {
            this.secureVault = secureVault;
        }
        this.logger = logger || (0, logger_1.createLogger)({ serviceName: 'OCCAMConsultancyAgent' });
        this.llmConfig = llmConfig;
        this.storageDir = storageDir || path_1.default.join(process.cwd(), 'storage', 'policies');
    }
    /**
     * Runs a comprehensive readiness check for an entity
     */
    async runReadinessCheck(entityId) {
        const traceId = this.auditService.generateTraceId();
        (0, logger_1.logWithTrace)(this.logger, 'info', `Starting readiness check for entity: ${entityId}`, traceId);
        try {
            // Fetch entity data
            const entity = await this.factBox.getEntityData(entityId);
            if (!entity) {
                await this.auditService.logFailure(traceId, 'readiness_check', 'Entity not found', { entity_id: entityId });
            ...
            (0, logger_1.logWithTrace)(this.logger, 'info', `Readiness check completed: ${overallReadiness} (score: ${readinessScore})`, traceId);
            return result;
        }
        catch (error) {
     ...
     */
    async generatePolicySummary(domain, jurisdiction) {
        const traceId = this.auditService.generateTraceId();
        (0, logger_1.logWithTrace)(this.logger, 'info', `Generating policy summary for domain: ${domain}`, traceId);
        try {
            ...
            if (rules.length === 0) {
                (0, logger_1.logWithTrace)(this.logger, 'warn', `No regulatory rules found for domain: ${domain}, jurisdiction: ${effectiveJurisdiction}`, traceId);
            }
            // Extract key requirements from rules
            const keyRequirements = rules.map((rule) => ({
            ...
            let sources = [`FactBox regulatory database - ${domain}`];
            ...
            (0, logger_1.logWithTrace)(this.logger, 'info', `Policy summary generated for ${domain}`, traceId);
     ...
     */
    async recommendNextSteps(reports) {
        const traceId = this.auditService.generateTraceId();
        (0, logger_1.logWithTrace)(this.logger, 'info', `Generating recommendations based on ${reports.length} compliance reports`, traceId);
        try {
            const recommendations = [];
            // Analyze patterns across reports
            const criticalFindings = reports.flatMap((r) => r.findings.filter((f) => f.severity === 'critical'));
            ...
            if (this.llmConfig && reports.length > 0) {
                ...
                if (aiRecommendations) {
                    ...
                    (0, logger_1.logWithTrace)(this.logger, 'info', 'AI-enhanced recommendations generated', traceId);
                }
            }
            // Log success
            ...
            (0, logger_1.logWithTrace)(this.logger, 'info', `Generated ${recommendations.length} improvement recommendations`, traceId);
     ...
     */
    async performRegistrationChecks(entity) {
        const checks = [];
     ...
     */
    async performDocumentationCheck(entity) {
     ...
     */
    async performLicenseChecks(entity) {
        const checks = [];
        // Check if entity has active licenses based on type
        const activeLicenses = entity.registrations.filter((reg) => reg.status === 'active' && reg.type.toLowerCase().includes('license'));
        checks.push({
            category: 'license',
     ...
     */
    async generateNextActions(checks, entity) {
        const actions = [];
        ...
        failedChecks.forEach((check) => {
            if (check.category === 'kyc') {
                ...
                actions.push({
                    ...
                    description: `Renew expired registrations: ${check.details?.expired_registrations?.map((r) => r.type).join(', ')}`,
            ...
            if (check.category === 'registration' && check.name === 'Expiring Registrations') {
                ...
                if (Array.isArray(expiringRegs) && expiringRegs.length > 0) {
                    actions.push({
                        ...
                        description: `Plan renewal for registrations expiring soon: ${expiringRegs.map((r) => r.type).join(', ')}`,
                        estimated_effort: 'medium',
                    });
                }
            }
     ...
     */
    async saveReadinessReport(result) {
        try {
            ...
            await promises_1.default.writeFile(filepath, markdown, 'utf-8');
            (0, logger_1.logWithTrace)(this.logger, 'info', `Readiness report saved: ${filename}`, result.trace_id);
        }
        catch (error) {
            (0, logger_1.logWithTrace)(this.logger, 'error', `Failed to save readiness report: ${error instanceof Error ? error.message : 'Unknown error'}`, result.trace_id);
        }
    }
    /**
     * Generates markdown formatted readiness report
     */
    generateReadinessMarkdown(result) {
     ...
     */
    async savePolicySummary(summary) {
        try {
            await promises_1.default.mkdir(this.storageDir, { recursive: true });
            const timestamp = new Date().toISOString().split('T')[0];
            ...
            await promises_1.default.writeFile(filepath, markdown, 'utf-8');
            (0, logger_1.logWithTrace)(this.logger, 'info', `Policy summary saved: ${filename}`, summary.trace_id);
        }
        catch (error) {
            (0, logger_1.logWithTrace)(this.logger, 'error', `Failed to save policy summary: ${error instanceof Error ? error.message : 'Unknown error'}`, summary.trace_id);
        }
    }
    /**
     * Generates markdown formatted policy summary
     */
    generatePolicySummaryMarkdown(summary) {
     ...
     */
    async generateAIRecommendations(entity, checks) {
        if (!this.llmConfig) {
            return null;
        }
        try {
            ...
            (0, logger_1.logWithTrace)(this.logger, 'info', 'AI recommendations generation is configured but not yet implemented', '');
            // Placeholder return
            return {
            ...
            (0, logger_1.logWithTrace)(this.logger, 'error', `AI recommendations failed: ${error instanceof Error ? error.message : 'Unknown error'}`, '');
            return null;
        }
    }
    /**
     ...
     */
    async generateAIPolicySummary(domain, jurisdiction, rules) {
        if (!this.llmConfig) {
            return null;
        }
        try {
            ...
            (0, logger_1.logWithTrace)(this.logger, 'error', `AI policy summary failed: ${error instanceof Error ? error.message : 'Unknown error'}`, '');
            return null;
        }
    }
    /**
     ...
     */
    async generateAIImprovementRecommendations(reports) {
        if (!this.llmConfig) {
            return null;
        }
        try {
            ...
            (0, logger_1.logWithTrace)(this.logger, 'error', `AI improvement recommendations failed: ${error instanceof Error ? error.message : 'Unknown error'}`, '');
            return null;
        }
    }
}
exports.OCCAMConsultancyAgent = OCCAMConsultancyAgent;
//# sourceMappingURL=occam-consultancy-agent.js.map